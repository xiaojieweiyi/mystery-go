		...
		trees: map[string]*node{
			http.MethodGet: {
				...
				children: map[string]*node{
					...
					"order": {
						path: "order", children: map[string]*node{
							"detail":   {path: "detail", handler: mockHandler},
							"abstract": {path: "abstract", handler: mockHandler},
						},
					},
				},
				handler: mockHandler},
			http.MethodPost: {
				...
				children: map[string]*node{
					"order": {
						...
						children: map[string]*node{
							"create": {
								path: "create", handler: mockHandler,
							},
						},
					},
					"login": {
...
func Test_router_AddRouter_V2(t *testing.T) {
	testRoutes := []struct {
		method string
		path   string
	}{
	...
	//
	//// 多个 /
	assert.PanicsWithValue(t, "web: 非法路由。不允许使用 //a/b, /a//b之类的路由，[/a//b]", func() {
...
//func Test_router_findRoute_V2(t *testing.T) {
//	testRoutes := []struct {
//		method string
//		path   string
//	}{
...
func Test_router_AddRoute_V3(t *testing.T) {
	testRoutes := []struct {
		method string
		path   string
	}{
	...
	assert.PanicsWithValue(t, "web: 非法路由，已有通配符路由。不允许同时注册通配符路由和参数路由 [:id]", func() {
		r.addRouter(http.MethodGet, "/a/*", mockHandler)
		r.addRouter(http.MethodGet, "/a/:id", mockHandler)
	})
	assert.PanicsWithValue(t, "web: 非法路由，已有路径参数路由。不允许同时注册通配符路由和参数路由 [*]", func() {
		r.addRouter(http.MethodGet, "/a/b/:id", mockHandler)
		r.addRouter(http.MethodGet, "/a/b/*", mockHandler)
	})
	r = newRouter()
	assert.PanicsWithValue(t, "web: 非法路由，已有通配符路由。不允许同时注册通配符路由和参数路由 [:id]", func() {
		r.addRouter(http.MethodGet, "/*", mockHandler)
		r.addRouter(http.MethodGet, "/:id", mockHandler)
	})
	r = newRouter()
	assert.PanicsWithValue(t, "web: 非法路由，已有路径参数路由。不允许同时注册通配符路由和参数路由 [*]", func() {
		r.addRouter(http.MethodGet, "/:id", mockHandler)
	...
	assert.PanicsWithValue(t, "web: 路由冲突，参数路由冲突，已有 :id，新注册 :name", func() {
		r.addRouter(http.MethodGet, "/a/b/c/:id", mockHandler)
		r.addRouter(http.MethodGet, "/a/b/c/:name", mockHandler)
	})
}
